...
def get_lidar_timestamps(data_folder_path):
    ...
    if os.path.exists(
            os.path.join(data_folder_path, "compensated_pc/timestamps.txt")):
        timestamp_filepaths_dict["compensated_pc"] = os.path.join(
            data_folder_path, "compensated_pc/timestamps.txt")
...
def load_stereo_calib(camera_fn, stereo_fn):
    ...
    if not os.path.exists(camera_fn) or not os.path.exists(stereo_fn):
        print("Calibration files do not exist!", camera_fn, 'and ', stereo_fn)
        exit()
